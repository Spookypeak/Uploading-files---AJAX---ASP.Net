    ...
    fileInput.addEventListener('change', function () {
        ...
        getBase64(this.files[0], function (base64) {
            ajax(
                ...
                JSON.stringify({ name: name, extension: extension, date: date, data: base64 }),
                function () {
                    appendFile({ Id: JSON.parse(this.response).d, Name: name, Extension: extension, Date: date.getTime(), Data: base64 })
                });
        });
    });
...
function getFiles() {
    ajax(
        ...
        function () {
            ...
            if (JSON.parse(this.response).d)
                //Calls drawer method
                drawFiles(JSON.parse(this.response).d);
        }
    );
}
    ...
    files.map(function (file) {
        filesDiv.innerHTML += `
            <div id="${file.Id}" class="row">
                ...
                <div class="cell" style="width:24.5%;float:left;">${setTime(file.Date)}</div>
...
function appendFile(file) {
    ...
    filesDiv.innerHTML += `
            <div id="${file.Id}" class="row">
                ...
                <div class="cell" style="width:24.5%;float:left;">${file.Extension}</div>
                <div class="cell" style="width:24.5%;float:left;">
                    <a href="${file.Data}" class="button" name="${file.Id}" download="${file.Name}">Download file</a>
                </div>
            </div>
          `;
}
...
function getFile(idFile) {
    ajax(
        'POST',
        'Index.aspx/GetFile',
        JSON.stringify({ idFile: idFile }),
        function () {
            //Sends id and data file converted to base64 from byte array
            setFile(idFile, atob(JSON.parse(this.response).d));
        }
    );
}
